...
class SubscriptionCache {
  ...
  static Future<void> initializeTable() async {
    try {
      ...
      try {
        ...
        ''');
        debugPrint('✅ Added billing_period column to existing table');
      } catch (e) {
        // Column already exists or table was just created - ignore
        debugPrint('ℹ️ billing_period column already exists or table just created');
      }

      debugPrint('✅ Subscription cache table initialized');
    } catch (e) {
      debugPrint('❌ Error creating subscription cache table: $e');
    }
  }
  ...
  static Future<void> saveSubscriptionData({
    ...
    try {
      ...
      debugPrint('✅ Subscription data cached locally');
      debugPrint('   Status: $status');
      debugPrint('   Trial Days: $trialDays');
      debugPrint('   Billing Period: ${billingPeriod ?? "monthly"}'); // ✅ ADD
    } catch (e) {
      debugPrint('❌ Error caching subscription data: $e');
    }
  }
  ...
  static Future<Map<String, dynamic>?> getCachedSubscriptionData() async {
    try {
      ...
      if (results.isEmpty) {
        debugPrint('⚠️ No cached subscription data found');
        return null;
      }
      ...
      debugPrint('📦 Retrieved cached subscription data');
      debugPrint('   Status: ${cache['subscription_status']}');
      debugPrint('   Trial Days: ${cache['remaining_trial_days']}');
      debugPrint('   Last Synced: $daysSinceSync days ago');
      ...
      if (cache['subscription_expires_at'] != null) {
        ...
        if (now.isAfter(expiresAt)) {
          ...
          debugPrint('✅ Subscription valid until $expiresAt');
        }
      }
      ...
      debugPrint('❌ Error getting cached subscription data: $e');
      return null;
    }
  }
