...
class ProductionSyncEngine {
  ...
  static Future<DeviceSyncState> _loadSyncState(String schoolId) async {
    try {
      ...
      debugPrint('⚠️ Error loading sync state: $e');
    }
  ...
  static Future<void> _saveSyncState(DeviceSyncState state) async {
    try {
      ...
      debugPrint('💾 Sync state saved with UTC timestamps');
    } catch (e) {
      debugPrint('❌ Failed to save sync state: $e');
    }
  }
  ...
  static Future<SyncResult> smartSync({
    ...
    try {
      ...
      if (lastSync == null || now.difference(lastSync).inDays > 7) {
        debugPrint(
            '🎯 Strategy: FULL SYNC (${lastSync == null ? "Never synced" : "${now.difference(lastSync).inDays} days ago"})');
      ...
      if (now.difference(lastSync).inHours < 1) {
        debugPrint(
            '⏭️ Strategy: SKIP (Last sync was ${now.difference(lastSync).inMinutes} minutes ago)');
        return SyncResult(
          true,
      ...
      debugPrint(
          '🎯 Strategy: INCREMENTAL SYNC (${now.difference(lastSync).inHours} hours ago)');
  ...
  static Future<SyncResult> performProductionSync({
    ...
    try {
      ...
      debugPrint('🎯 Sync Strategy: ${strategy.name}');
      ...
      debugPrint('💥 Production sync failed: $e');
      debugPrint('📚 Stack trace: $stackTrace');
  ...
  static Future<void> _registerDeviceInBackground(
    ...
    try {
      ...
      debugPrint('⚠️ Background device registration failed (non-critical): $e');
    }
  }
  ...
  static Future<SyncResult> _executeFullSync(DeviceSyncState state) async {
    try {
      debugPrint('⬇️ Executing full sync...');

      debugPrint('📥 Downloading all school data...');
      final response =
      ...
      debugPrint('📤 Uploading pending changes...');
  ...
  static Future<SyncResult> _executeIncrementalSyncFixed(
    ...
    debugPrint('⚡ Executing incremental sync...');

    try {
      final lastSyncTime = currentState.lastIncrementalSync;

      debugPrint('📥 Downloading changes since $lastSyncTime...');
      ...
      final downloadedCount = _countRecords(serverData);
      debugPrint('📊 Received $downloadedCount records');

      if (downloadedCount == 0) {
        if (await _shouldForceFullSync(lastSyncTime)) {
          debugPrint('🔄 Smart Fallback: Switching to full sync...');
      ...
      debugPrint('🔄 Falling back to full sync...');
      return await _executeFullSync(currentState);
    }
  }
          ...
          for (var item in rawList) {
            if (item is Map) {
              ...
              if (tableName == 'users') {
                ...
                // Server requires email for users. Keep local optional behavior
                // for instructors by supplying a deterministic sync-only placeholder.
                if (role == 'instructor' && email.isEmpty) {
                  final idPart = (data['id'] ?? '').toString().trim();
                  final phoneDigits = (data['phone'] ?? '')
                      .toString()
                  ...
                  final safeSchoolId =
                      schoolId.trim().replaceAll(RegExp(r'[^0-9a-zA-Z]'), '');

                  data['email'] =
                      'instructor.$safeSchoolId.$stablePart@local.invalid';
                }
              }
  ...
  static Future<void> _updateLocalIds(Map<String, dynamic> idMappings) async {
    debugPrint('🔄 Processing Server ID mappings...');
