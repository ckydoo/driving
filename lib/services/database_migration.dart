...
class DatabaseMigration {
  ...
  static Future<void> runMigrations(Database db) async {
    ...
    try {
      ...
      if (verification['all_tables_have_timestamps']) {
        ...
        debugPrint('🎉 Database ready for sync with timestamp fields');
      } else {
        debugPrint('⚠️ === MIGRATIONS COMPLETED WITH WARNINGS ===');
        debugPrint(
  ...
  static Future<void> _runExistingMigrations(Database db) async {
    debugPrint('🔧 Running existing migrations...');
    ...
    try {
      ...
      debugPrint('✅ Existing migrations completed');
    } catch (e) {
      debugPrint('❌ Existing migrations failed: $e');
      throw e;
    }
  }
  ...
  static Future<void> _addMultiTenantSupport(Database db) async {
    debugPrint('🏫 Adding multi-tenant support...');
    ...
    try {
      ...
      if (!userColumns.contains('school_id')) {
        ...
        debugPrint('✅ Added school_id to users table');
      } else {
        debugPrint('ℹ️ school_id already exists in users table');
      }
        ...
        try {
          ...
          if (result.isNotEmpty) {
            ...
            if (!columns.contains('school_id')) {
              ...
              debugPrint('✅ Added school_id to $tableName table');
            } else {
              debugPrint('ℹ️ school_id already exists in $tableName');
            }
          }
        } catch (e) {
          debugPrint('⚠️ Could not update $tableName table: $e');
          // Continue with other tables
        }
      }

      debugPrint('✅ Multi-tenant support added');
    } catch (e) {
      debugPrint('❌ Error adding multi-tenant support: $e');
  ...
  static Future<void> _createSafeIndexes(Database db) async {
    debugPrint('📈 Creating performance indexes for production...');
    ...
    try {
      ...
      for (String table in tables) {
        try {
          ...
          debugPrint('⚠️ Failed to create updated_at index for $table: $e');
        }
      }

      // CRITICAL: Multi-tenant indexes (school_id on all tables)
      debugPrint('📊 Creating multi-tenant indexes...');
      ...
      debugPrint('🚀 Creating query performance indexes...');
      ...
      debugPrint('⚡ Creating composite indexes...');
      ...
      debugPrint('✅ All performance indexes created successfully');
    } catch (e) {
      debugPrint('❌ Failed to create indexes: $e');
    }
  }
  ...
  static Future<void> _createIndexIfColumnExists(
    ...
    try {
      ...
      debugPrint('⚠️ Failed to create index $indexName: $e');
    }
  }
  ...
  static Future<void> _createCompositeIndexIfColumnsExist(
    ...
    try {
      ...
      if (allColumnsExist) {
        ...
        debugPrint('✅ Created composite index $indexName on ($columnList)');
      } else {
      ...
      debugPrint('⚠️ Failed to create composite index $indexName: $e');
    }
  }
  ...
  static Future<void> _createSchoolsTable(Database db) async {
    ...
    debugPrint('✅ Schools table created/verified');
  }
  ...
  Future<void> runFullMigration() async {
    try {
      debugPrint('🔄 Running full migration (compatibility mode)...');
      debugPrint('✅ Full migration completed');
    } catch (e) {
      debugPrint('❌ Full migration failed: $e');
    }
  }
}
