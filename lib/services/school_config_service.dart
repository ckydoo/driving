...
class SchoolConfigService extends GetxService {
  ...
  Future<void> initializeSchoolConfig() async {
    try {
      debugPrint('🏫 Initializing school configuration...');
      ...
      debugPrint('✅ School configuration initialized:');
      debugPrint('   School ID: ${schoolId.value}');
      debugPrint('   School Name: ${schoolName.value}');
    } catch (e) {
      debugPrint('❌ Failed to initialize school configuration: $e');
      // Set fallback values
  ...
  Future<void> _ensureSettingsLoaded() async {
    ...
    if (_settingsController.businessName.value.isEmpty) {
      debugPrint('📋 Loading business settings...');
  ...
  String getCollectionPath(String collection) {
    if (!isInitialized.value || schoolId.value.isEmpty) {
      debugPrint('⚠️ School not initialized, using default path for $collection');
  ...
  Future<void> updateSchoolConfig() async {
    debugPrint('🔄 Updating school configuration...');
    ...
    if (oldSchoolId != schoolId.value) {
      debugPrint('⚠️ School ID changed from $oldSchoolId to ${schoolId.value}');
      debugPrint('   This may require data migration in Firebase');
  ...
  void _handleSchoolIdentityChange(String oldSchoolId, String newSchoolId) {
    ...
    debugPrint('🚨 SCHOOL IDENTITY CHANGE DETECTED:');
    debugPrint('   Old School ID: $oldSchoolId');
    debugPrint('   New School ID: $newSchoolId');
    debugPrint('   Consider data migration if needed');
  }
