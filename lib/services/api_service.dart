...
class ApiService {
  ...
  static void configure({required String baseUrl}) {
    ...
    debugPrint('🔧 API Service configured: $baseUrl');
  }
  ...
  static Future<Map<String, dynamic>> getSchoolSyncState({
    ...
    return _withRetry(() async {
      debugPrint(
          '🏫 Getting sync state for school: $schoolId, device: $deviceId');
  ...
  static Future<List<dynamic>> downloadTableData({
    ...
    return _withRetry(() async {
      debugPrint('📊 Downloading table data: $table for school: $schoolId');
      ...
      debugPrint('✅ Downloaded $table: ${result.length} records');
      return result;
    });
  }
  ...
  static Future<Map<String, dynamic>> getSyncStatus({
    ...
    return _withRetry(() async {
      debugPrint('📊 Getting sync status for school: $schoolId');
  ...
  static Future<Map<String, dynamic>> syncDownload({String? lastSync}) async {
    return _withRetry(() async {
      debugPrint('🔍 Making legacy sync download request...');
  ...
  static Future<Map<String, dynamic>> syncUpload(dynamic changes) async {
    return _withRetry(() async {
      debugPrint('📤 Making legacy sync upload request...');
      debugPrint('🔍 Changes type: ${changes.runtimeType}');
      debugPrint(
      ...
      if (changes is Map<String, dynamic>) {
        ...
        debugPrint('📋 Using List format: ${changes.length} items');
      } else {
      ...
      return {
        ...
        'uploaded': responseData['uploaded'] ?? 0,
        'errors': responseData['errors'] ?? [],
        'message': data['message'] ?? 'Upload completed',
        'partial': responseData['partial'] ?? false,
        'timestamp': responseData['timestamp'],
        'id_mappings': responseData['id_mappings'] ?? data['id_mappings'] ?? {},
      };
    });
  }
    ...
    try {
      ...
      try {
        ...
        if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
          debugPrint('✅ Internet connection confirmed (DNS)');
          return true;
        }
      } catch (e) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          debugPrint('✅ Internet connection confirmed (HTTP)');
          return true;
        }
      } catch (e) {
        debugPrint('⚠️ HTTP connectivity test failed: $e');
      }
  ...
  static Map<String, dynamic> _handleResponse(http.Response response) {
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      try {
        ...
        developer.log('JSON decode error',
            error: e, name: 'ApiService', level: 1000);
        throw ApiException(
            response.statusCode, 'Received invalid data from server');
      }
    }
    ...
    try {
      ...
      developer.log('Could not parse error response',
          error: e, name: 'ApiService');
    }
  ...
  static Future<T> _withRetry<T>(Future<T> Function() operation) async {
    ...
    while (attempts < maxRetries) {
      ...
      try {
        developer.log('Request attempt $attempts/$maxRetries',
            name: 'ApiService');
        return await operation();
      } catch (e) {
        developer.log('Request failed (attempt $attempts)',
            error: e, name: 'ApiService');

        // Don't retry on authentication errors
        if (e is ApiException && e.statusCode == 401) {
          developer.log('Not retrying authentication error',
              name: 'ApiService');
          rethrow;
        }
        ...
        if (attempts >= maxRetries) {
          developer.log('All retry attempts exhausted',
              name: 'ApiService', level: 1000);
          rethrow;
        }
  ...
  static Future<void> logout() async {
    try {
      ...
      debugPrint('⚠️ Logout request failed (continuing anyway): $e');
    } finally {
      clearToken();
    }
  }
...
// 2. Incremental (Points to ProductionSyncController@downloadIncrementalChanges)
  static Future<Map<String, dynamic>> downloadIncrementalChanges(
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint('❌ Incremental download failed: ${response.statusCode}');
      debugPrint('Response body: ${response.body}');
