...
class AppInitialization {
  static Future<void> initialize() async {
    ...
    try {
      debugPrint('📁 Initializing database helper...');
      Get.put(DatabaseHelper.instance, permanent: true);
      debugPrint('✅ Database helper initialized');
      ...
      debugPrint('❌ === CRITICAL ERROR DURING APP INITIALIZATION ===');
      debugPrint('Error: $e');
  ...
  static Future<void> _runDatabaseMigration() async {
    try {
      debugPrint('🔧 Running database migration...');
  ...
  static Future<void> _runScheduleStatusMigration() async {
    try {
      debugPrint('📊 Running schedule status migration...');
      await ScheduleStatusMigration.instance.runStatusMigration();
      debugPrint('✅ Schedule status migration completed');
    } catch (e) {
      debugPrint('❌ Schedule status migration failed: $e');
  ...
  static Future<void> _initializeControllers() async {
    try {
      ...
      debugPrint('  ✅ Navigation controller initialized');

      // Initialize data controllers
      Get.put(UserController(), permanent: true);
      debugPrint('  ✅ User controller initialized');

      Get.put(CourseController(), permanent: true);
      debugPrint('  ✅ Course controller initialized');

      Get.put(FleetController(), permanent: true);
      debugPrint('  ✅ Fleet controller initialized');

      // Initialize operational controllers
      Get.put(ScheduleController(), permanent: true);
      debugPrint('  ✅ Schedule controller initialized');

      Get.put(BillingController(), permanent: true);
      debugPrint('  ✅ Billing controller initialized');
      Get.put(SettingsController(), permanent: true);
      debugPrint('  ✅ Settings controller initialized');
  ...
  static Future<Map<String, dynamic>> getAppHealthStatus() async {
    ...
    try {
      ...
      debugPrint('Controllers health check failed: $e');
    }
    ...
    try {
      ...
      debugPrint('Migration health check failed: $e');
    }
