...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 2) {
      ...
      try {
        ...
        ''');
        debugPrint('✅ Added billing_period column to subscription_cache table');
      } catch (e) {
        debugPrint(
            '⚠️ Error adding billing_period column (may already exist): $e');
      }
    }

    if (oldVersion < 3) {
      // Migration to version 3: add payment voiding columns for backward compatibility
      try {
        await db.execute(
            'ALTER TABLE payments ADD COLUMN is_voided INTEGER NOT NULL DEFAULT 0');
        debugPrint('✅ Added is_voided column to payments table');
      } catch (e) {
        debugPrint('⚠️ Error adding is_voided column (may already exist): $e');
      }

      try {
        await db.execute('ALTER TABLE payments ADD COLUMN voided_at TEXT');
        debugPrint('✅ Added voided_at column to payments table');
      } catch (e) {
        debugPrint('⚠️ Error adding voided_at column (may already exist): $e');
      }
    }

    if (oldVersion < 4) {
      // Migration v4: add school_id to fleet, invoices, courses for multi-tenant sync
      for (final table in ['fleet', 'invoices', 'courses']) {
        try {
          await db.execute('ALTER TABLE $table ADD COLUMN school_id TEXT');
          debugPrint('✅ Added school_id to $table');
        } catch (e) {
          debugPrint('⚠️ school_id on $table may already exist: $e');
        }
      }
    }

    if (oldVersion < 5) {
      // Migration v5: make users.email optional (still unique when provided)
      await _migrateUsersTableAllowOptionalEmail(db);
    }
  }

  Future<void> _migrateUsersTableAllowOptionalEmail(Database db) async {
    try {
      await db.transaction((txn) async {
        ...
        await txn.execute('''
          INSERT INTO users_new (
            id, fname, lname, email, gender, date_of_birth, phone, idnumber,
            address, password, course, role, courseIds, school_id, status,
            created_at, last_login, last_login_method, is_super_admin
          )
          SELECT
            id, fname, lname, email, gender, date_of_birth, phone, idnumber,
            address, password, course, role, courseIds, school_id, status,
      ...
      debugPrint('✅ Migrated users table: email is now optional');
    } catch (e) {
      debugPrint('⚠️ Users table migration skipped/failed: $e');
    }
  }

  Future<Set<String>> _getTableColumns(String tableName) async {
    final cached = _tableColumnsCache[tableName];
  ...
  Future<Map<String, dynamic>> _filterToExistingColumns(
    ...
    final cols = await _getTableColumns(tableName);
...
''');
  ...
  Future<int> insertUser(Map<String, dynamic> user) async {
    try {
      ...
      if (email.isEmpty && role != 'instructor') {
      ...
      debugPrint('📝 Inserting user data: ${user['email'] ?? '(no email)'}');

      // Check for existing user BEFORE inserting
      final duplicateWhere = <String>[];
      final duplicateArgs = <dynamic>[];

      final idNumber = (user['idnumber']?.toString().trim() ?? '');
      if (idNumber.isNotEmpty) {
        duplicateWhere.add('idnumber = ?');
        duplicateArgs.add(idNumber);
      }

      if (user['email'] != null) {
        duplicateWhere.add('email = ?');
        duplicateArgs.add(user['email']);
      }

      final existingUsers = duplicateWhere.isEmpty
          ? <Map<String, dynamic>>[]
          : await db.query(
              'users',
              where: duplicateWhere.join(' OR '),
      ...
      if (existingUsers.isNotEmpty) {
        ...
        debugPrint(
            'User already exists: ${existing['email']}, updating instead...');
        ...
        if (updatedRows > 0) {
          debugPrint('✅ User updated successfully: ${existing['id']}');
      ...
      debugPrint('✅ User inserted successfully with ID: $insertedId');
      return insertedId;
    } catch (e) {
  ...
  Future<int> updateUser(Map<String, dynamic> user) async {
    try {
      ...
      debugPrint(
          '📝 Updating user data: ID ${user['id']}, Email ${user['email'] ?? '(no email)'}');
      ...
      debugPrint('✅ User updated successfully: $rowsAffected row(s) affected');
      return rowsAffected;
    } catch (e) {
  ...
  Future<int> insertInvoice(Map<String, dynamic> invoice) async {
    final db = await database;
    final insertData = await _filterToExistingColumns('invoices', invoice);

    if ((insertData['school_id'] == null ||
            insertData['school_id'].toString().isEmpty) &&
        (await _getTableColumns('invoices')).contains('school_id')) {
      final currentSchoolId = await getCurrentSchoolId();
      if (currentSchoolId != null && currentSchoolId.isNotEmpty) {
        insertData['school_id'] = currentSchoolId;
      }
    }
    ...
    return await db.insert('invoices', insertData);
  }

  Future<int> updateInvoice(Map<String, dynamic> invoice) async {
    debugPrint('DatabaseHelper: Updating invoice with data: $invoice');
    final db = await database;
    final invoiceId = invoice['id'];
    final updateData = await _filterToExistingColumns('invoices', invoice);
    updateData.remove('id');

    return await db.update('invoices', updateData,
  ...
  Future<int> insertPayment(Map<String, dynamic> payment) async {
    ...
    final insertData = await _filterToExistingColumns('payments', payment);
    // Avoid forcing null primary key into insert statement on some SQLite builds.
    if (insertData['id'] == null) {
      insertData.remove('id');
    }

    return await db.insert('payments', insertData);
  }
  ...
  Future<int> updatePayment(Map<String, dynamic> payment) async {
    ...
    final updateData = await _filterToExistingColumns('payments', payment);
    updateData.remove('id');

    return await db.update('payments', updateData,
    ...
    try {
      ...
      if (settingsResult.isNotEmpty) {
        ...
        if (schoolId != null && schoolId.isNotEmpty) {
          debugPrint('🔍 Found school ID in settings: $schoolId');
          return schoolId;
        }
      }
      ...
      if (schoolsResult.isNotEmpty) {
        ...
        debugPrint('🔍 Found school ID in schools table: $schoolId');
      ...
      debugPrint('⚠️ No school ID found in any table');
      return null;
    } catch (e) {
      debugPrint('❌ Error getting current school ID: $e');
      return null;
    }
  }
  ...
  Future<void> setCurrentSchoolId(String schoolId) async {
    try {
      ...
      debugPrint('✅ School ID saved to settings: $schoolId');
    } catch (e) {
      debugPrint('❌ Error setting current school ID: $e');
    }
  }
  ...
  Future<bool> schoolExistsInTable(String schoolId) async {
    try {
      ...
      debugPrint('❌ Error checking school existence: $e');
      return false;
    }
  }
  ...
  Future<Map<String, dynamic>?> getSchoolById(String schoolId) async {
    try {
      ...
      debugPrint('❌ Error getting school by ID: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> getSchoolByInvitationCode(String code) async {
    try {
      ...
      debugPrint('❌ Error getting school by invitation code: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> getSchoolByName(String name) async {
    try {
      ...
      debugPrint('❌ Error getting school by name: $e');
      return null;
    }
  }
  ...
  Future<void> insertSchool(Map<String, dynamic> schoolData) async {
    try {
      ...
      debugPrint('✅ School inserted/updated: ${schoolData['name']}');
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getActiveSchools() async {
    try {
      ...
      debugPrint('❌ Error getting active schools: $e');
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>?> getUserByEmailAndSchool(
    ...
    try {
      ...
      debugPrint('❌ Error getting user by email and school: $e');
      return null;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getUsersForSchool(String schoolId) async {
    try {
      ...
      debugPrint('❌ Error getting users for school: $e');
      return [];
    }
  }
  ...
  Future<bool> schoolHasAdmins(String schoolId) async {
    try {
      ...
      debugPrint('❌ Error checking school admins: $e');
      return false;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getStudentsForCurrentSchool() async {
    try {
      ...
      if (schoolId == null) {
        debugPrint('⚠️ No current school set');
        return [];
      }
      ...
      debugPrint('❌ Error getting students for current school: $e');
      return [];
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getInstructorsForCurrentSchool() async {
    try {
      ...
      debugPrint('❌ Error getting instructors for current school: $e');
      return [];
    }
  }
  ...
  Future<bool> isFirstRunCompleted() async {
    try {
      ...
      debugPrint('❌ Error checking first run status: $e');
      return false;
    }
  }
  ...
  Future<void> markFirstRunCompleted() async {
    try {
      ...
      debugPrint('✅ First run marked as completed');
    } catch (e) {
      debugPrint('❌ Error marking first run completed: $e');
    }
  }
  ...
  Future<void> clearAllSchoolData() async {
    try {
      ...
      debugPrint('🗑️ === CLEARING ALL SCHOOL DATA ===');

      // Core tables
      await db.delete('users');
      debugPrint('   ✅ users cleared');

      await db.delete('courses');
      debugPrint('   ✅ courses cleared');

      await db.delete('coursesenrolled');
      debugPrint('   ✅ coursesenrolled cleared');

      await db.delete('courseinstructor');
      debugPrint('   ✅ courseinstructor cleared');

      await db.delete('schedules');
      debugPrint('   ✅ schedules cleared');

      await db.delete('fleet');
      debugPrint('   ✅ fleet cleared');

      await db.delete('invoices');
      debugPrint('   ✅ invoices cleared');

      await db.delete('payments');
      debugPrint('   ✅ payments cleared');

      await db.delete('billings');
      debugPrint('   ✅ billings cleared');

      await db.delete('billing_records');
      debugPrint('   ✅ billing_records cleared');

      await db.delete('billing_records_history');
      debugPrint('   ✅ billing_records_history cleared');

      await db.delete('notes');
      debugPrint('   ✅ notes cleared');
      ...
      debugPrint('   ✅ timeline cleared');

      await db.delete('usermessages');
      debugPrint('   ✅ usermessages cleared');

      await db.delete('attachments');
      debugPrint('   ✅ attachments cleared');

      await db.delete('currencies');
      debugPrint('   ✅ currencies cleared');

      await db.delete('subscription_cache');
      debugPrint('   ✅ subscription_cache cleared');
      ...
      debugPrint('✅ === ALL SCHOOL DATA CLEARED ===');
    } catch (e) {
      debugPrint('❌ Error clearing school data: $e');
  ...
  Future<void> resetToFirstRun() async {
    try {
      ...
      debugPrint('✅ Reset to first run state');
    } catch (e) {
      debugPrint('❌ Error resetting to first run: $e');
    }
  }
  ...
  Future<void> saveKnownSchool(Map<String, dynamic> schoolData) async {
    ...
    try {
      ...
      debugPrint('✅ Saved known school: ${schoolData['name']}');
    } catch (e) {
      debugPrint('❌ Error saving known school: $e');
      throw e;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getKnownSchools() async {
    ...
    try {
      ...
      debugPrint('❌ Error getting known schools: $e');
      return [];
    }
  }
  ...
  Future<void> clearKnownSchools() async {
    ...
    try {
      ...
      debugPrint('✅ Cleared all known schools');
    } catch (e) {
      debugPrint('❌ Error clearing known schools: $e');
      throw e;
    }
  }
  ...
  Future<void> migratePrinterSettings() async {
    ...
    try {
      debugPrint('🔄 Migrating printer settings...');
      ...
      if (existingSettings.isEmpty) {
        ...
        debugPrint('✅ Printer settings migrated successfully');
      } else {
        debugPrint('ℹ️ Printer settings already exist, skipping migration');
      }
    } catch (e) {
      debugPrint('❌ Error migrating printer settings: $e');
    }
  }
}
