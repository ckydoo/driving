...
class SyncDebugHelper {
  ...
  static Future<void> logActivity(String activity,
    ...
    try {
      ...
      debugPrint('🐛 DEBUG: [$timestamp] $activity');
      if (details != null && details.isNotEmpty) {
        debugPrint('🐛 Details: ${json.encode(details)}');
      }
    } catch (e) {
      debugPrint('❌ Failed to log sync activity: $e');
    }
  }
  ...
  static Future<void> logError(
    ...
    try {
      ...
      debugPrint('🚨 SYNC ERROR: [$timestamp] $error');
      if (context != null) {
        debugPrint('🚨 Context: $context');
      }
    } catch (e) {
      debugPrint('❌ Failed to log sync error: $e');
    }
  }
  ...
  static Future<List<Map<String, dynamic>>> getDebugLogs() async {
    try {
      ...
      debugPrint('❌ Failed to get debug logs: $e');
      return [];
    }
  }
  ...
  static Future<Map<String, dynamic>?> getLastError() async {
    try {
      ...
      debugPrint('❌ Failed to get last error: $e');
      return null;
    }
  }
  ...
  static Future<void> clearDebugData() async {
    try {
      ...
      debugPrint('🧹 Debug data cleared');
    } catch (e) {
      debugPrint('❌ Failed to clear debug data: $e');
    }
  }
  ...
  static Future<void> printDiagnosticReport() async {
    debugPrint('\n' + '=' * 50);
    debugPrint('🔍 SYNC DIAGNOSTIC REPORT');
    debugPrint('=' * 50);

    final report = await generateDiagnosticReport();

    debugPrint('📅 Generated: ${report['timestamp']}');
    debugPrint('🔄 Last Sync: ${report['last_sync']}');
    debugPrint('📊 Pending Changes: ${report['pending_changes']['details']}');
    ...
    if (report['last_error'] != null) {
      ...
      debugPrint('   Context: ${lastError['context'] ?? 'N/A'}');
      debugPrint('   Time: ${lastError['timestamp']}');
    }

    debugPrint('📝 Recent Activity (${report['recent_activity'].length} items):');
    final recentActivity = report['recent_activity'] as List<dynamic>;
    for (final activity in recentActivity.reversed.take(5)) {
      debugPrint('   ${activity['timestamp']}: ${activity['activity']}');
    }
