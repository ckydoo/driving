...
class _SubscriptionCheckScreenState extends State<SubscriptionCheckScreen> {
  ...
  Future<void> _checkSubscriptionAndNavigate() async {
    try {
      ...
      debugPrint('🔐 === APP STARTUP SUBSCRIPTION CHECK ===');
      debugPrint('Target route: ${widget.nextRoute}');
      ...
      debugPrint('🔄 Loading subscription status from server...');
      ...
      debugPrint('📊 Trial Days Remaining: $trialDays');

      // Check if subscription allows access
      if (status == 'suspended') {
        debugPrint('🚫 BLOCKED: Subscription is suspended');
        _showBlockedScreen('suspended');
        return;
      }

      if (status == 'expired') {
        debugPrint('🚫 BLOCKED: Subscription is expired');
        _showBlockedScreen('expired');
        return;
      }

      if (status == 'trial' && trialDays <= 0) {
        debugPrint('🚫 BLOCKED: Trial has expired');
        _showBlockedScreen('trial_expired');
        return;
      }

      // All checks passed - allow access
      debugPrint('✅ ALLOWED: Subscription is valid ($status, $trialDays days)');
      debugPrint('✅ Navigating to: ${widget.nextRoute}');
