...
class _CourseScreenState extends State<CourseScreen>
  ...
  void initState() {
    ...
    // Keep local screen snapshot in sync with controller updates.
    _coursesWorker = ever<List<Course>>(controller.visibleCourses, (_) {
      if (!mounted) return;
      setState(() {
  ...
  Widget _buildResponsiveFAB(BuildContext context) {
    if (_isMobile(context)) {
      return FloatingActionButton.extended(
        onPressed: () async {
          await Get.dialog<bool>(CourseFormDialog());
          await _loadCourses();
        },
  ...
  Widget _buildCourseListTile(Course course) {
    ...
    return ListTile(
      ...
      onTap: () {
        Get.find<NavigationController>().openCourseDetails(course.id!);
      },
