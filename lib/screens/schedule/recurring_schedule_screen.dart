...
class _RecurringScheduleScreenState extends State<RecurringScheduleScreen> {
  ...
  final TextEditingController _instructorSearchController =
      TextEditingController();
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          leading: CircleAvatar(
                            radius: 16,
                            backgroundColor:
                                isSelected ? Colors.blue : Colors.blue.shade300,
                            child: Text(
...
// Helper method to get next date based on recurrence pattern
  ...
  Future<void> _loadStudentCourses(User student) async {
    ...
    try {
      ...
      debugPrint('ERROR in _loadStudentCourses: $e');
      Get.snackbar(
        snackPosition: SnackPosition.BOTTOM,
        'Error',
...
// DEBUGGING METHOD: Add this to help troubleshoot
  void debugBillingAndSchedules(int studentId, int courseId) {
    ...
    debugPrint('=== BILLING DEBUG ===');
    debugPrint('Student ID: $studentId, Course ID: $courseId');
    debugPrint('Invoice found: ${invoice != null}');
    if (invoice != null) {
      debugPrint('Invoice lessons: ${invoice.lessons}');
    }
    debugPrint(
    ...
    debugPrint('Used lessons: $usedLessons');
    debugPrint('Remaining lessons: $remainingLessons');
...
// FIXED: Calculate the perfect end date for user's remaining lessons
  DateTime _calculateOptimalEndDate() {
    ...
    debugPrint('🔄 OPTIMAL DATE CALCULATION:');
    debugPrint('   Remaining lessons: $_remainingLessons');
    debugPrint('   Lessons per occurrence: $lessonsPerOccurrence');
    debugPrint('   Needed occurrences: $neededOccurrences');
...
// FIXED: Update preview count with better logging
  void _updatePreviewCount() {
    ...
    debugPrint('🔄 UPDATE PREVIEW COUNT:');
    debugPrint(
        '   Start date: ${_selectedStartDate.toString().substring(0, 10)}');
    debugPrint('   End date: ${endDate.toString().substring(0, 10)}');
    debugPrint('   Pattern: $_recurrencePattern');
    debugPrint('   Selected days: $_selectedDaysOfWeek');
    debugPrint('   Use end date: $_useEndDate');
    debugPrint('   Max occurrences: $_maxOccurrences');
    ...
    debugPrint('   Final preview count: $count');
    setState(() {
      _previewCount = count;
    });
...
// FIXED: Smart end date selection with guidance
  Widget _buildEndConditionSection() {
    ...
    debugPrint('🔄 END CONDITION SECTION:');
    debugPrint('   Lessons per session: $lessonsPerSession');
    debugPrint('   Remaining lessons: $_remainingLessons');
    debugPrint('   Max sessions: $maxSessions');
    debugPrint(
        '   Optimal end date: ${optimalEndDate.toString().substring(0, 10)}');
...
// ALSO ADD this debug method to help troubleshoot
  void _debugCurrentState() {
    ...
    debugPrint('   _remainingLessons: $_remainingLessons');
    debugPrint('   _selectedStartDate: ${_selectedStartDate.toString()}');
    debugPrint('   _startTime: ${_startTime.toString()}');
    debugPrint('   _endTime: ${_endTime.toString()}');
    debugPrint('   _recurrencePattern: $_recurrencePattern');
    debugPrint('   _selectedDaysOfWeek: $_selectedDaysOfWeek');
    debugPrint('   _customInterval: $_customInterval');
    debugPrint('   _useEndDate: $_useEndDate');
    debugPrint('   _recurrenceEndDate: ${_recurrenceEndDate?.toString()}');
    debugPrint('   _maxOccurrences: $_maxOccurrences');
    debugPrint('   _previewCount: $_previewCount');
    debugPrint('   _getLessonsPerOccurrence(): ${_getLessonsPerOccurrence()}');
    debugPrint('   _getTotalLessonsToDeduct(): ${_getTotalLessonsToDeduct()}');

    final errors = _getValidationErrors();
    debugPrint('   Validation errors: $errors');

    final warnings = _getValidationWarnings();
    debugPrint('   Validation warnings: $warnings');
  }
...
// HELPER METHOD: Validate lesson duration and show helpful messages
              ...
              child: _filteredInstructors.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final canSelect =
                            _selectedClassType != 'Practical' || hasVehicle;
            ...
            return FilterChip(
              ...
              onSelected: (selected) {
                setState(() {
                  ...
                  debugPrint('🔄 Day selection updated: $_selectedDaysOfWeek');
                  debugPrint(
  ...
  Future<void> _checkInstructorAndVehicleAvailability() async {
    ...
    try {
      ...
      debugPrint('🔄 Checking conflicts for ${scheduleDates.length} dates');
  ...
  Future<void> _createRecurringSchedule() async {
    debugPrint('🔄 RECURRING SCHEDULE CREATION STARTED');
    ...
    try {
      ...
      debugPrint('   Days checked: $daysChecked');
      debugPrint('   Schedules generated: ${schedules.length}');
      debugPrint('   Schedules skipped: ${skippedDates.length}');
      debugPrint('   Target was: $targetScheduleCount');
      ...
      for (int i = 0; i < schedules.length; i++) {
        try {
          ...
          debugPrint('❌ Failed to save schedule ${i + 1}: $e');
        }
      }

      debugPrint('🔄 SAVE RESULTS:');
      debugPrint('   Successfully saved: $savedCount');
      debugPrint('   Failed to save: $failedCount');
      debugPrint('   Conflicts skipped: ${skippedDates.length}');
      ...
      debugPrint('🚨 ERROR in recurring schedule creation: $e');
  ...
  bool _canCreateRecurringSchedule() {
    if (_selectedStudent == null ||
        ...
        _previewCount <= 0 ||
        _remainingLessons <= 0) {
      debugPrint('DEBUG: Basic requirements not met');
      return false;
    }
    ...
    if (_selectedClassType == 'Practical') {
      if (_selectedVehicle == null) {
        debugPrint('DEBUG: _selectedVehicle is null for practical lesson');
        return false;
      }

      // Ensure the vehicle is actually assigned to the selected instructor
      if (_selectedVehicle!.instructor != _selectedInstructor!.id) {
        debugPrint('DEBUG: Vehicle not assigned to instructor');
        return false;
      }
    }
    ...
    if (startDay.isBefore(today)) {
      debugPrint('DEBUG: Start date is in the past');
      return false;
    }
    ...
    if (startDay.isAtSameMomentAs(today)) {
      ...
      if (startDateTime.isBefore(now)) {
        debugPrint('DEBUG: Start time is in the past');
        return false;
      }
    }
    ...
    final errors = _getValidationErrors();
    if (errors.isNotEmpty) {
      debugPrint('DEBUG: Validation errors: $errors');
      return false;
    }
  ...
  void _testConflictDetection() {
    debugPrint('🔧 TESTING CONFLICT DETECTION');
    debugPrint(
    ...
    debugPrint('   Pattern: $_recurrencePattern');
    debugPrint('   Selected days: $_selectedDaysOfWeek');
    debugPrint('   Show availability status: $_showAvailabilityStatus');
    debugPrint('   Instructor available: $_instructorAvailable');
    debugPrint('   Conflict details: $_conflictDetails');
