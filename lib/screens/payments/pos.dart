...
class _POSScreenState extends State<POSScreen> {
  ...
  void _loadUsers() async {
    try {
      ...
      debugPrint('✅ Loaded ${userController.users.length} users successfully');

      // Debug: Show user roles after loading
      if (userController.users.isNotEmpty) {
        debugPrint('👥 User roles available:');
        ...
        roleGroups
            .forEach((role, count) => debugPrint('  - "$role": $count users'));
      }
    } catch (e) {
...
// 2. FIXED: Safe quantity update with proper validation
  void _updateCartQuantity(int index, int newQuantity) {
    ...
    if (index < 0 || index >= _cartItems.length) {
      debugPrint('Invalid index: $index, cart length: ${_cartItems.length}');
      return;
    }
    ...
    for (final item in _cartItems) {
      try {
        ...
        final invoice = Invoice(
          ...
          // Start at zero; recordPayment will apply the payment once.
          amountPaid: 0.0,
        ...
        debugPrint(
            'Found created invoice with database ID: ${createdInvoice!.id}');
        ...
        debugPrint('Creating payment with invoice ID: ${payment.invoiceId}');

        // Record payment
        await billingController.recordPayment(payment);

        debugPrint(
            'Payment recorded successfully for invoice ${createdInvoice.id}');
      } catch (e) {
        debugPrint('❌ Error processing payment for ${item.course.name}: $e');
        throw Exception(
  ...
  Future<void> _createInvoiceOnly() async {
    ...
    for (final item in _cartItems) {
      try {
        ...
        debugPrint('Invoice created successfully: $invoiceNumber');
        ...
        if (shouldAutoPrint) {
          try {
            ...
            debugPrint('⚠️ Invoice created but print failed: $e');
        ...
        debugPrint('❌ Error creating invoice for ${item.course.name}: $e');
...
// ENHANCED: Better success display
  void _showSuccess(String message) {
    debugPrint('✅ POS Success: $message');
  ...
  Future<void> _processPaymentAndPrint() async {
    ...
    try {
      ...
      if (shouldAutoPrint) {
        // Auto-print receipt
        try {
          debugPrint('🖨️ Attempting to print receipt...');
          ...
          _showSuccess(
              'Payment processed! Print failed - you can print manually from receipts.');
        }
      } else {
  ...
  void _showInvoicePrintDialog(String invoiceNumber, CartItem item) {
    Get.dialog(
      AlertDialog(
        ...
        content:
            Text('Would you like to print the invoice for this transaction?'),
        actions: [
          TextButton(
            onPressed: () => Get.back(),
            child: Text('No'),
