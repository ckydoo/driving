...
class _UsersReportsScreenState extends State<UsersReportsScreen> {
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'This preview shows live data from your current records. Use "Print" to send directly to your printer or "Export PDF" to save the complete report.',
                      style: TextStyle(
  ...
  List<Map<String, dynamic>> _getInstructorWorkloadData() {
    ...
    return instructors.map((instructor) {
      final instructorId = instructor.id ?? -1;
      final instructorSchedules = scheduleController.schedules
          .where((s) => s.instructorId == instructorId)
          .toList();

      final studentCount =
          instructorSchedules.map((s) => s.studentId).toSet().length;
      final last7Days = DateTime.now().subtract(const Duration(days: 7));
      final weeklyLessons = instructorSchedules
          .where((s) =>
              s.start.isAfter(last7Days) &&
              s.status.toLowerCase() != 'cancelled')
          .length;
      final currentMonth =
          DateTime(DateTime.now().year, DateTime.now().month, 1);
      final monthlySchedules = instructorSchedules
          .where((s) =>
              s.start.isAfter(currentMonth) &&
              s.status.toLowerCase() != 'cancelled')
          .toList();
      final monthlyHours = monthlySchedules.fold<double>(
          0.0, (sum, s) => sum + (s.end.difference(s.start).inMinutes / 60.0));
  ...
  List<Map<String, dynamic>> _getRegistrationTrendsData() {
    ...
    // Count actual registrations by role and month
    for (final user in users) {
      final key = DateFormat('MMM yyyy').format(
        DateTime(user.created_at.year, user.created_at.month, 1),
      );
      if (!months.containsKey(key)) continue;

      final role = user.role.toLowerCase();
      if (role == 'student') {
        months[key]!['students'] = (months[key]!['students'] ?? 0) + 1;
      } else if (role == 'instructor') {
        months[key]!['instructors'] = (months[key]!['instructors'] ?? 0) + 1;
      }
      months[key]!['total'] = (months[key]!['total'] ?? 0) + 1;
    }

    int previousTotal = 0;
    return monthOrder.map((key) {
      final entry =
          months[key] ?? {'students': 0, 'instructors': 0, 'total': 0};
      final students = entry['students'] ?? 0;
      final instructors = entry['instructors'] ?? 0;
  ...
  List<Map<String, dynamic>> _getInstructorPerformanceData() {
    ...
    return instructors.map((instructor) {
      ...
      final nonCancelled = schedules
          .where((s) => s.status.toLowerCase() != 'cancelled')
          .toList();
      final studentsTaught =
          nonCancelled.map((s) => s.studentId).toSet().length;
      final attendedLessons = nonCancelled.where((s) => s.attended).length;
      final totalLessons = nonCancelled.length;
      final passRate =
          totalLessons > 0 ? (attendedLessons / totalLessons) * 100 : 0.0;

      final currentMonth =
          DateTime(DateTime.now().year, DateTime.now().month, 1);
      final lessonsThisMonth =
          nonCancelled.where((s) => s.start.isAfter(currentMonth)).length;

      // Use an attendance-backed score mapped to a 1-5 scale.
      final rating = (1.0 + (passRate / 100.0) * 4.0).clamp(1.0, 5.0);
  ...
  List<Map<String, dynamic>> _getUserActivityData() {
    final users = userController.users.take(10).toList();
    return users.map((user) {
      final userSchedules = scheduleController.schedules
          .where((s) => s.studentId == user.id || s.instructorId == user.id)
          .toList();
      final userInvoices = billingController.invoices
          .where((inv) => inv.studentId == user.id)
          .toList();
      final invoiceIds = userInvoices.map((e) => e.id).whereType<int>().toSet();
      final userPayments = billingController.payments
          .where((p) => invoiceIds.contains(p.invoiceId))
          .toList();

      final activityCandidates = <DateTime>[user.created_at]
        ..addAll(userSchedules.map((s) => s.start))
        ..addAll(userInvoices.map((i) => i.createdAt))
        ..addAll(userPayments.map((p) => p.paymentDate));

      activityCandidates.sort((a, b) => b.compareTo(a));
      final lastActivity = activityCandidates.first;
      final daysSinceActivity = DateTime.now().difference(lastActivity).inDays;

      final loginFrequency = daysSinceActivity < 3
          ? 'Daily'
          : daysSinceActivity < 7
              ? 'Weekly'
      ...
      return {
        ...
        'role': user.role,
        'lastLogin': DateFormat('MMM d, yyyy').format(lastActivity),
