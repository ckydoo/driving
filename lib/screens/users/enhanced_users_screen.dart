...
class _EnhancedUsersScreenState extends State<EnhancedUsersScreen>
    ...
    _recommendations = [
      {
        ...
        'action': () => navController.openAddUser(widget.role),
        'color': Colors.blue,
      },
      {
        ...
        'action': () => navController.openBulkStudentUpload(),
        'color': Colors.green,
      },
    ];
  }
      ...
      children: [
        ...
        Padding(
          ...
          child: Column(
            children: [
              for (int index = 0;
                  index < _getPaginatedResults().length;
                  index++) ...[
                _buildUserCard(_getPaginatedResults()[index], isMobile),
                if (index < _getPaginatedResults().length - 1)
                  SizedBox(height: 12),
              ],
            ],
          ),
  ...
  void _handleMenuAction(String action, User user) {
    switch (action) {
      ...
      case 'graduate':
        if (widget.role == 'student') {
          Get.find<NavigationController>().openStudentGraduation(user);
        }
        break;
      case 'delete':
  ...
  void _navigateToUserDetails(User user) {
    if (widget.role == 'student') {
      Get.find<NavigationController>().openStudentDetails(user.id!);
    } else if (widget.role == 'instructor') {
      Get.find<NavigationController>().openInstructorDetails(user.id!);
    }
  }
