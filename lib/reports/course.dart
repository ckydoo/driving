...
class _CourseReportsScreenState extends State<CourseReportsScreen> {
  ...
  List<Map<String, dynamic>> _generateCourseReportData() {
    ...
    for (var course in courseController.courses) {
      // Filter schedules within the selected date range
      ...
      // Revenue is derived from real invoice payments for this course in range.
      final courseInvoices = billingController.invoices
          .where((inv) =>
      ...
      final revenue = courseInvoices.fold<double>(
        0.0,
        (sum, inv) => sum + inv.amountPaid,
      );

      // Keep course in report if it has either operational activity or billed activity.
      if (courseSchedules.isEmpty && courseInvoices.isEmpty) {
        continue;
      }

      final studentCount = {
        ...courseSchedules.map((s) => s.studentId),
        ...courseInvoices.map((i) => i.studentId),
      }.length;
      final totalLessons = courseSchedules.length;
      final completedLessons = courseSchedules.where((s) => s.attended).length;
      final progress =
