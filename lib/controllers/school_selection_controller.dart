...
class SchoolSelectionController extends GetxController {
  ...
  Future<void> joinSchool() async {
    ...
    try {
      ...
      try {
        ...
        _showLoadingDialog();
        debugPrint('🔄 Trying offline fallback authentication...');
        await _joinSchoolOffline(schoolName, email, password);
      } catch (offlineError) {
        _closeLoadingDialog();
        Get.snackbar(
    ...
    try {
      ...
      if (isAuthenticated) {
        ...
        if (userData.isNotEmpty) {
          ...
          debugPrint('✅ User authenticated offline: $email');
        }
  ...
  Future<void> _saveOnlineResultLocally(Map<String, dynamic> result) async {
    try {
      debugPrint('🔍 === SAVING ONLINE RESULT ===');
      debugPrint('Result keys: ${result.keys.toList()}');
      debugPrint('School data: ${result['school']}');
      debugPrint('User data: ${result['user']}');
      ...
      debugPrint('💾 Saving school: ID=$schoolId, Name=${school['name']}');
      ...
      debugPrint('💾 Saving user: ID=$userId, Email=${user['email']}');
      ...
      debugPrint('✅ Online result saved locally for offline access');
    } catch (e) {
      debugPrint('❌ Failed to save online result locally: $e');
  ...
  Future<void> _joinSchoolOnline(
    ...
    try {
      ...
      debugPrint('🔍 API Result received: ${result.keys.toList()}');
      ...
      debugPrint('✅ User authenticated after joining: ${result['user']['email']}');
      ...
      _closeLoadingDialog();
      debugPrint('❌ Online authentication failed: $e');
      debugPrint('❌ Stack trace: ${StackTrace.current}');
  ...
  Future<void> ensureSchoolDataPersistence(Map<String, dynamic> school) async {
    try {
      debugPrint('🔧 Ensuring school data persistence...');
      final db = await _dbHelper.database;
      ...
      if (schoolCheck.isEmpty) {
        debugPrint('⚠️ School not in schools table, inserting...');
      ...
      if (settingsCheck.isEmpty ||
        ...
        debugPrint('⚠️ School ID not in settings, inserting...');
        await db.insert(
          'settings',
      ...
      if (businessNameCheck.isEmpty) {
        debugPrint('⚠️ Business name not in settings, inserting...');
        await db.insert(
          'settings',
      ...
      debugPrint('✅ School data persistence verified');
      await _verifySchoolDataSaved(schoolIdString);
    } catch (e) {
      debugPrint('❌ Error ensuring school data persistence: $e');
  ...
  Future<void> _verifySchoolDataSaved(String schoolId) async {
    try {
      ...
      debugPrint('   Schools table: ${schoolResult.isNotEmpty ? "✅" : "❌"}');
      debugPrint('   Settings table entries: ${settingsResult.length}');
  ...
  Future<void> _setCurrentSchool(Map<String, dynamic> school) async {
    try {
      ...
      debugPrint('❌ Error setting current school: $e');
  ...
  Future<void> _ensureSchoolsTableExists() async {
    try {
      ...
      ''');
      debugPrint('✅ Schools table ensured');
    } catch (e) {
      debugPrint('❌ Error creating schools table: $e');
    }
  }
  ...
  static Future<bool> isFirstRun() async {
    try {
      ...
      if (settings.isNotEmpty) {
        ...
        debugPrint('🔍 First run completed flag: $isCompleted');

        if (isCompleted) {
          // Double-check we have school data
          final hasSchoolData = await _hasValidSchoolData(db);
          debugPrint('🔍 Has valid school data: $hasSchoolData');
      ...
      final hasSchoolData = await _hasValidSchoolData(db);
      debugPrint('🔍 Has school data (no flag): $hasSchoolData');
    ...
    try {
      ...
      if (businessNameResult.isNotEmpty) {
        ...
        if (businessName != null && businessName.isNotEmpty) {
          debugPrint('✅ Found business name in settings: $businessName');
          return true;
        }
      }
      ...
      if (schoolsResult.isNotEmpty) {
        ...
        debugPrint('✅ Found school in schools table: $schoolName');
        return true;
      }

      return false;
    } catch (e) {
      debugPrint('❌ Error checking school data: $e');
      return false;
    }
  }
