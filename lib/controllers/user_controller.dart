...
class UserController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    ever(visibleUsers,
        (_) => debugPrint('Users list updated: ${visibleUsers.length} users'));
  }
  ...
  Future<void> _loadInitialUsers() async {
    try {
      ...
      debugPrint(
          '✅ Loaded ${visibleUsers.length} users (hasMore: ${hasMoreUsers.value})');
    } catch (e) {
      debugPrint('Error loading initial users: $e');
      rethrow;
    }
  }
  ...
  Future<void> _loadInitialStudents() async {
    try {
      ...
      debugPrint(
          '✅ Loaded ${visibleStudents.length} students (hasMore: ${hasMoreStudents.value})');
    } catch (e) {
      debugPrint('Error loading initial students: $e');
      rethrow;
    }
  }
  ...
  Future<void> _loadInitialInstructors() async {
    try {
      ...
      debugPrint(
          '✅ Loaded ${visibleInstructors.length} instructors (hasMore: ${hasMoreInstructors.value})');
    } catch (e) {
      debugPrint('Error loading initial instructors: $e');
      rethrow;
    }
  }
  ...
  Future<void> loadMoreUsers() async {
    ...
    try {
      ...
      debugPrint(
          '✅ Loaded ${result['users'].length} more users (total: ${visibleUsers.length})');
    } catch (e) {
  ...
  Future<void> loadMoreStudents() async {
    ...
    try {
      ...
      debugPrint(
          '✅ Loaded ${result['users'].length} more students (total: ${visibleStudents.length})');
    } catch (e) {
  ...
  Future<void> loadMoreInstructors() async {
    ...
    try {
      ...
      debugPrint(
          '✅ Loaded ${result['users'].length} more instructors (total: ${visibleInstructors.length})');
    } catch (e) {
      debugPrint('Error loading more instructors: $e');
      Get.snackbar(
        'Error',
  ...
  Future<List<User>> fetchUsers({String? role}) async {
    debugPrint(
        'UserController: fetchUsers called with role: $role (redirecting to lazy loading)');
    ...
    try {
      ...
      error(e.toString());
      debugPrint('UserController: Error fetching users - ${e.toString()}');
      Get.snackbar(
        snackPosition: SnackPosition.BOTTOM,
  ...
  Future<List<User>> refreshUsersForRole(String role) async {
    debugPrint('UserController: Refreshing users for role: $role');
  ...
  Future<Map<String, String>> checkForDuplicates(
    ...
    try {
      ...
      for (final existingUserData in allUsers) {
        ...
        // Check for duplicate email only when both users have an email
        if (existingUser.email.trim().isNotEmpty &&
  ...
  Future<void> _handleUserUpdate(User user) async {
    ...
    if (userMap['school_id'] == null) {
      userMap['school_id'] = '1';
      debugPrint('⚠️ Applied fallback school_id: 1');
    }

    debugPrint('📝 Updating user with school_id: ${userMap['school_id']}');

    await DatabaseHelper.instance.updateUser(userMap);

    await SyncService.trackChange('users', userMap, 'update');
    debugPrint('📝 Tracked user update for sync');
    ...
    debugPrint('✅ User updated locally successfully');
  ...
  Future<void> _handleNewUserCreation(User user) async {
    ...
    debugPrint('📝 Tracked user creation for sync');

    _users.add(createdUser);
    debugPrint('✅ User saved locally with ID: $newUserId');
    Get.snackbar(
      snackPosition: SnackPosition.BOTTOM,
      'Success',
  ...
  Future<void> deleteUser(int userId) async {
    try {
      ...
      debugPrint('📝 Tracked user deletion for sync');
      ...
      debugPrint('UserController: User deleted successfully');
  ...
  Future<void> deleteMultipleUsers(List<int> userIds) async {
    try {
      ...
      debugPrint('UserController: Deleting ${userIds.length} users');

      for (int id in userIds) {
        await SyncService.trackChange('users', {'id': id}, 'delete');
        debugPrint('📝 Tracked user deletion for sync: $id');
      ...
      debugPrint(
          'UserController: Successfully deleted ${userIds.length} users');
      ...
      error(e.toString());
      debugPrint(
          'UserController: Error deleting multiple users - ${e.toString()}');
      Get.snackbar(
        snackPosition: SnackPosition.BOTTOM,
