...
class SchoolRegistrationController extends GetxController {
  ...
  Future<void> _checkInternetAndSetMode() async {
    try {
      debugPrint('🔍 Checking internet connectivity for registration...');
      ...
      if (online) {
        ...
        debugPrint('🌐 Internet available - Online registration ready');
      } else {
        registrationMode.value = 'online_required';
        debugPrint('📵 No internet - Online registration required for first setup');
      }
    } catch (e) {
  ...
  Future<void> registerSchool() async {
    ...
    try {
      ...
      debugPrint('🌐 === INTERNET-FIRST SCHOOL REGISTRATION ===');
      ...
      debugPrint('❌ School registration failed: $e');
  ...
  Future<Map<String, dynamic>> _registerSchoolOnline() async {
    debugPrint('📡 Step 1: Registering school online...');
    ...
    try {
      ...
      debugPrint('✅ Step 1 Complete: School registered online');
      return result;
    } catch (e) {
      debugPrint('❌ Step 1 Failed: Online registration failed - $e');
  ...
  Future<void> _downloadAndSaveSchoolData(Map<String, dynamic> result) async {
    debugPrint('💾 Step 2: Downloading school data locally...');
    ...
    ''');
      ...
      debugPrint('✅ Step 2 Complete: School data saved locally');
    } catch (e) {
      debugPrint('❌ Step 2 Failed: Failed to save school data - $e');
  ...
  Future<void> _setupLocalAuthentication(Map<String, dynamic> result) async {
    debugPrint('🔐 Step 3: Setting up local authentication...');
    ...
    ''');
      ...
      debugPrint('✅ Step 3 Complete: Local authentication setup');
    } catch (e) {
      debugPrint('❌ Step 3 Failed: Failed to setup local auth - $e');
  ...
  Future<void> _updateAppSettings(Map<String, dynamic> result) async {
    debugPrint('⚙️ Step 4: Updating app settings...');
    ...
    try {
      ...
      debugPrint('✅ Step 4 Complete: App settings updated');
    } catch (e) {
      debugPrint('❌ Step 4 Failed: Failed to update settings - $e');
  ...
  Future<void> _showSuccessAndNavigate(Map<String, dynamic> result) async {
    debugPrint('🎉 Step 5: Registration complete - redirecting to PIN setup...');
    ...
    try {
      ...
      debugPrint('📊 Registration result keys: ${result.keys.toList()}');
      debugPrint('👤 User data: $adminUser');
      debugPrint('🏫 School data: $school');
      ...
      if (adminUser != null && school != null) {
        ...
        debugPrint('⚠️ Warning: User or school data is null');
        debugPrint('   adminUser: $adminUser');
      ...
      debugPrint('✅ Step 5 Complete: User ready for PIN setup or main app');
    } catch (e) {
      debugPrint('❌ Step 5 Failed: Navigation error - $e');
  ...
  void _navigateToPinSetup() async {
    ...
    try {
      ...
      debugPrint('🔍 Pre-navigation auth check:');
      ...
      if (!authController.isLoggedIn.value ||
        ...
        debugPrint('⚠️ Auth state not ready - waiting longer...');
      ...
      if (authController.isLoggedIn.value &&
          authController.currentUser.value != null) {
        debugPrint('✅ Auth state confirmed - navigating to PIN setup');
        Get.offAllNamed('/pin-setup');
      } else {
        debugPrint('❌ Auth state not properly set - falling back to main');
        _navigateToMain();
      }
    } catch (e) {
      debugPrint('❌ Navigation to PIN setup failed: $e');
  ...
  void _navigateToMain() {
    debugPrint('🏠 Navigating to main application...');
    try {
      // Clear all previous routes and go to main
      AppRoutes.toMain();
    } catch (e) {
      debugPrint('❌ Navigation to main failed: $e');
  ...
  Future<void> _autoLoginUser(
    ...
    try {
      debugPrint('🔐 Auto-logging in registered user...');
      ...
      debugPrint('✅ User set in AuthController:');
      debugPrint('   isLoggedIn: ${authController.isLoggedIn.value}');
      debugPrint('   currentUser: ${authController.currentUser.value?.email}');
      debugPrint('   role: ${authController.currentUser.value?.role}');
      ...
      if (ApiService.hasToken && ApiService.currentToken != null) {
        ...
        debugPrint('✅ API token saved to SharedPreferences: $tokenKey');
        debugPrint('   Token: ${ApiService.currentToken!.substring(0, 10)}...');
      } else {
        debugPrint(
      ...
      debugPrint('🔄 Triggering initial sync after registration...');
      await _triggerPostRegistrationSync();

      // Mark first run as completed
      await SchoolSelectionController.markFirstRunCompleted();

      debugPrint('✅ User auto-logged in successfully');
    } catch (e, stackTrace) {
  ...
  Future<void> _triggerPostRegistrationSync() async {
    try {
      debugPrint('🔄 === POST-REGISTRATION AUTO-SYNC ===');

      if (!Get.isRegistered<SyncController>()) {
        debugPrint('⚠️ SyncController not registered yet');
        return;
      }
      ...
      debugPrint('✅ Triggering auto-sync after registration...');

      // Delay to ensure everything is initialized
      Future.delayed(Duration(milliseconds: 1000), () {
        if (Get.isRegistered<SyncController>()) {
          Get.find<SyncController>().performFullSync();
          debugPrint('✅ Auto-sync triggered after registration');
        }
      });
    } catch (e) {
      debugPrint('❌ Failed to trigger post-registration sync: $e');
