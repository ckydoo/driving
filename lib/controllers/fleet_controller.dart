...
class FleetController extends GetxController {
  ...
  Future<void> _loadInitialFleet() async {
    try {
      ...
      debugPrint('✅ Loaded ${visibleFleet.length} vehicles (hasMore: ${hasMore.value})');
    } catch (e) {
      debugPrint('Error loading initial fleet: $e');
      rethrow;
    }
  }
  ...
  Future<void> loadMoreFleet() async {
    ...
    try {
      ...
      debugPrint('✅ Loaded ${result['fleet'].length} more vehicles (total: ${visibleFleet.length})');
    } catch (e) {
      debugPrint('Error loading more fleet: $e');
      Get.snackbar(
        'Error',
  ...
  Future<void> searchFleetLazy(String query) async {
    ...
    if (query.isEmpty) {
      ...
      try {
        ...
        debugPrint('Error searching fleet: $e');
      }
    }
  }
  ...
  Future<void> fetchFleet() async {
    debugPrint('FleetController: fetchFleet called (redirecting to lazy loading)');
    await refreshFleet();
  }
  ...
  Future<void> handleFleet(Fleet vehicle, {bool isUpdate = false}) async {
    try {
      ...
      if (isUpdate) {
        ...
        debugPrint('✅ Vehicle created successfully with ID: $vehicleId');
      }
      ...
      try {
        ...
        debugPrint('📝 Tracked vehicle change for sync');
      } catch (e) {
        debugPrint('⚠️ Sync tracking failed (non-critical): $e');
      }
      ...
      try {
        ...
        debugPrint('⚠️ Fleet refresh failed (non-critical): $e');
      }
      ...
      try {
        ...
        debugPrint('⚠️ Snackbar failed (non-critical): $e');
      }
    } catch (e) {
      error('Fleet operation failed: ${e.toString()}');
      debugPrint('❌ handleFleet error: $e');
  ...
  Future<void> deleteFleet(int id) async {
    try {
      ...
      debugPrint('📝 Tracked vehicle deletion for sync');
      ...
      debugPrint('❌ deleteFleet error: $e');
  ...
  Future<void> assignVehicleToInstructor(
    ...
    try {
      debugPrint('🚗 Assigning vehicle $vehicleId to instructor $instructorId');
      ...
      debugPrint('📝 Tracked vehicle assignment for sync');

      // Refresh fleet data
      await fetchFleet();

      debugPrint('✅ Vehicle assigned successfully');
  ...
  Future<void> updateVehicleStatus(int vehicleId, String status) async {
    try {
      ...
      // 🔄 TRACK THE CHANGE FOR SYNC
      await SyncService.trackChange('fleet', updateData, 'update');
      debugPrint('📝 Tracked vehicle status update for sync');
  ...
  Future<void> updateVehicleField(
    ...
    try {
      debugPrint('🚗 Updating vehicle field: $field for vehicle ID: $vehicleId');
      ...
      // 🔄 TRACK THE CHANGE FOR SYNC
      await SyncService.trackChange('fleet', updateData, 'update');
      debugPrint('📝 Tracked vehicle field update for sync');

      // Refresh fleet
      await fetchFleet();

      debugPrint('✅ Vehicle field updated successfully');
    } catch (e) {
      debugPrint('❌ Error updating vehicle field: $e');
      Get.snackbar(
        'Error',
  ...
  Future<void> bulkDeleteVehicles(List<int> vehicleIds) async {
    try {
      isLoading(true);
      debugPrint('🚗 Bulk deleting ${vehicleIds.length} vehicles');
  ...
  Future<void> saveFleet(List<Fleet> newFleet) async {
    try {
      isLoading(true);
      debugPrint('🚗 Saving ${newFleet.length} vehicles to database');
      ...
      debugPrint('📝 Tracked ${newFleet.length} vehicle creations for sync');
      ...
      debugPrint('❌ Error saving fleet: $e');
      Get.snackbar(
        'Import Failed',
