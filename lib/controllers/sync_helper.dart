...
class SyncHelper {
      ...
      if (value != null && value.toString().contains('Timestamp(seconds=')) {
        try {
          ...
          if (secondsMatch != null) {
            ...
            debugPrint('🔄 Converted timestamp $key: $value -> $milliseconds');
          }
        } catch (e) {
          debugPrint('⚠️ Failed to convert timestamp $key: $e');
  ...
  static Future<int?> safeInsert(
    ...
    try {
      ...
      debugPrint('✅ Inserted into $table: ${convertedData['id'] ?? 'new record'}');
      return result;
    } catch (e) {
      debugPrint('❌ Error inserting into $table: $e');
      debugPrint('📋 Failed data: ${data.toString().substring(0, 200)}...');
      return null;
    }
  }
    ...
    try {
      ...
      for (final column in missingColumns) {
        try {
          ...
          debugPrint('✅ Added missing column: $table.$column ($columnType)');
        } catch (e) {
          debugPrint('⚠️ Could not add column $table.$column: $e');
        }
      }
    } catch (e) {
      debugPrint('⚠️ Error checking table structure for $table: $e');
    }
  }
  ...
  static Future<void> cleanDuplicates(
    ...
    try {
      ...
      debugPrint('⚠️ Error cleaning duplicates in $table: $e');
    }
  }
}
