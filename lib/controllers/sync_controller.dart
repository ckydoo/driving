...
class SyncController extends GetxController {
  ...
  Future<void> _initializeController() async {
    try {
      debugPrint('🔄 Initializing SyncController...');
      ...
      debugPrint('❌ SyncController initialization failed: $e');
    }
  }
  ...
  void _startSyncIfReady() {
    ...
    try {
      ...
      debugPrint('⚠️ AuthController not ready yet, will start sync on login');
    }
  }
  ...
  String _formatLastSyncForDisplay(String? isoTimestamp) {
    ...
    try {
      ...
      debugPrint('⚠️ Failed to parse last sync timestamp: $e');
      return 'Never';
    }
  }
  ...
  void startPeriodicSync() {
    if (_isDisposed || !_isInitialized) {
      debugPrint('⚠️ Cannot start periodic sync: controller not ready');
      return;
    }

    if (!autoSyncEnabled.value) {
      debugPrint('ℹ️ Auto-sync is disabled');
      return;
    }

    if (syncIntervalMinutes.value < 5) {
      debugPrint('⚠️ Sync interval too small, setting to minimum 5 minutes');
  ...
  void stopSync() {
    ...
    debugPrint('🛑 All sync activities stopped');
  }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (isPartial && errors.isNotEmpty) {
          ...
          debugPrint('⚠️ Partial upload detected - running auto-debug...');
        ...
        debugPrint('❌ Upload failed - running auto-debug...');
        await debugSyncStatus();
      }
    } catch (e) {
      if (!_isDisposed) {
        ...
        debugPrint('💥 Upload exception - running auto-debug...');
        await debugSyncStatus();
      }
    } finally {
      if (!_isDisposed) {
  ...
  Future<void> performProductionSync({bool forceFullSync = false}) async {
    ...
    try {
      ...
      debugPrint('🚀 Starting production sync for school: $schoolId');
      ...
      if (result.success) {
        ...
        debugPrint('✅ Production sync completed:');
        debugPrint('   Strategy: ${details['strategy'] ?? 'unknown'}');
        debugPrint('   Downloaded: ${details['downloaded_records'] ?? 0} records');
        debugPrint('   Uploaded: ${details['uploaded_changes'] ?? 0} changes');
        ...
        debugPrint('❌ Production sync failed: ${result.message}');
        await debugSyncStatus();
      }
    } catch (e) {
      if (!_isDisposed) {
  ...
  Future<void> performSmartSync() async {
    try {
      ...
      if (hasProductionSync) {
        debugPrint('🚀 Using production sync engine');
        await performProductionSync();
      } else {
        debugPrint('⚠️ Falling back to legacy sync');
        await performFullSync();
      }
    } catch (e) {
  ...
  Future<bool> _checkProductionSyncAvailability() async {
    try {
      ...
      debugPrint('⚠️ Production sync unavailable: $e');
      return false;
    }
  }
  ...
  Future<void> performFullSync({bool forceDownload = false}) async {
    ...
    try {
      ...
      if (result.success) {
        ...
        debugPrint('❌ Full sync failed - running auto-debug...');
        await debugSyncStatus();
      }
    } catch (e) {
      if (!_isDisposed) {
        ...
        debugPrint('💥 Full sync exception - running auto-debug...');
        await debugSyncStatus();
      }
    } finally {
      if (!_isDisposed) {
  ...
  Future<void> debugSyncStatus() async {
    ...
    debugPrint('🔍 Current pending changes status:');
    debugPrint('   Has pending: ${pendingStatus['has_pending']}');
    debugPrint('   Count: ${pendingStatus['count']}');
    debugPrint('   Details: ${pendingStatus['details']}');

    if (pendingStatus['breakdown'] != null) {
      final breakdown = pendingStatus['breakdown'] as Map<String, dynamic>;
      for (final entry in breakdown.entries) {
        debugPrint('   - ${entry.key}: ${entry.value} items');
      }
    }
  }
  ...
  Future<Map<String, dynamic>> getSyncInfo() async {
    try {
      ...
      debugPrint('❌ Failed to get sync info: $e');
      return {
        'connected': false,
        'last_sync': 'Unknown',
        'pending_changes': 0,
