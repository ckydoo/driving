...
class SettingsController extends GetxController {
  ...
  // Engagement feature flags (safe rollout: additive only)
  final RxBool enableReminderTemplates = false.obs;
  final RxBool enableRescheduleFromLink = false.obs;
  final RxBool enableOverdueBoard = false.obs;
  final RxBool enableInstructorDailyBriefing = false.obs;
...
// Lesson duration options (in hours)
  ...
  void onInit() {
    ...
    debugPrint('🚀 SettingsController onInit started');
    loadPrinterSettings();
    ...
    _initializeSettingsWithDefaults().then((_) {
      ...
      debugPrint('🎯 SettingsController fully initialized');
    }).catchError((e) {
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      enableReminderTemplates.value =
          prefs.getBool('enable_reminder_templates') ?? false;
      enableRescheduleFromLink.value =
          prefs.getBool('enable_reschedule_from_link') ?? false;
      enableOverdueBoard.value = prefs.getBool('enable_overdue_board') ?? false;
      enableInstructorDailyBriefing.value =
          prefs.getBool('enable_instructor_daily_briefing') ?? false;
  ...
  Future<void> ensureDefaultSettings() async {
    try {
      ...
      if (isFirstLaunch) {
        debugPrint('🎯 First launch detected - setting up default settings');
        ...
        debugPrint('✅ Default settings saved on first launch');
      }
    } catch (e) {
      debugPrint('❌ Error ensuring default settings: $e');
    }
  }
  ...
  Future<void> _saveDefaultSettings() async {
    try {
      ...
      await prefs.setBool('enable_reminder_templates', false);
      await prefs.setBool('enable_reschedule_from_link', false);
      await prefs.setBool('enable_overdue_board', false);
      await prefs.setBool('enable_instructor_daily_briefing', false);
      ...
      debugPrint('💾 All default settings saved to SharedPreferences');
    } catch (e) {
  ...
  void toggleReminderTemplates(bool value) => updateSetting(
      'enable_reminder_templates', value, enableReminderTemplates);

  void toggleRescheduleFromLink(bool value) => updateSetting(
      'enable_reschedule_from_link', value, enableRescheduleFromLink);

  void toggleOverdueBoard(bool value) =>
      updateSetting('enable_overdue_board', value, enableOverdueBoard);

  void toggleInstructorDailyBriefing(bool value) => updateSetting(
      'enable_instructor_daily_briefing', value, enableInstructorDailyBriefing);
  ...
  String exportSettings() {
    final settings = {
      ...
      'enable_reminder_templates': enableReminderTemplates.value,
      'enable_reschedule_from_link': enableRescheduleFromLink.value,
      'enable_overdue_board': enableOverdueBoard.value,
      'enable_instructor_daily_briefing': enableInstructorDailyBriefing.value,
  ...
  Future<void> resetToDefaults() async {
    try {
      ...
      enableReminderTemplates.value = false;
      enableRescheduleFromLink.value = false;
      enableOverdueBoard.value = false;
      enableInstructorDailyBriefing.value = false;
  ...
  Map<String, dynamic> getAllSettings() {
    return {
      ...
      'notifications': {
        ...
        'enableReminderTemplates': enableReminderTemplates.value,
        'enableRescheduleFromLink': enableRescheduleFromLink.value,
        'enableOverdueBoard': enableOverdueBoard.value,
        'enableInstructorDailyBriefing': enableInstructorDailyBriefing.value,
      },
      'app': {
        'theme': theme.value,
...
// Individual setters for text fields
  ...
  void _onBusinessInfoChanged(String field, String value) {
    debugPrint('📝 Business info changed: $field = $value');
    ...
    try {
      ...
      debugPrint('⚠️ Could not update school config: $e');
    }
  }
  ...
  void _updateSchoolConfigStatus() {
    ...
    if (isComplete) {
      ...
      debugPrint('✅ School configuration is complete');
    } else {
      debugPrint('⚠️ School configuration is incomplete');
    }
  }
  ...
  Future<void> saveBusinessSettings() async {
    try {
      debugPrint('💾 Starting to save enhanced business settings...');
      ...
      debugPrint('📋 Settings to save: $businessSettingsMap');

      for (final entry in businessSettingsMap.entries) {
        debugPrint('💾 Saving: ${entry.key} = ${entry.value}');
        await db.insert(
          'settings',
      ...
      debugPrint('✅ All enhanced business settings saved successfully');
    } catch (e) {
      debugPrint('❌ Error saving enhanced business settings: $e');
      rethrow;
    }
  }
  ...
  Future<void> loadSettingsFromDatabase() async {
    try {
      debugPrint('📖 Loading enhanced settings from database...');
      final db = await _dbHelper.database;
      ...
      debugPrint('✅ Enhanced settings loaded successfully');
    } catch (e) {
      debugPrint('❌ Error loading enhanced settings: $e');
      rethrow;
    }
  }
    ...
    try {
      ...
      if (schoolId.value.isNotEmpty) {
        ...
        try {
          ...
          debugPrint('⚠️ No current school ID found: $e');
        }
      }

      if (currentSchoolId != null && currentSchoolId.isNotEmpty) {
        debugPrint('🔍 Checking schools table for ID: $currentSchoolId');
        ...
        if (schoolRecords.isNotEmpty) {
          ...
          debugPrint('🏫 Found school record: ${schoolData['name']}');
          ...
          if (schoolData['operating_days'] != null) {
            ...
            debugPrint('✅ Updated operating days: ${operatingDays.value}');
          }
          ...
          debugPrint('💾 Saving merged school data to settings...');
          await _saveMergedData();

          debugPrint('✅ Successfully merged ALL school data into settings');
        } else {
          debugPrint('⚠️ No school record found for ID: $currentSchoolId');
        }
      } else {
        debugPrint('ℹ️ No school ID available for merging');
      }
    } catch (e) {
      debugPrint('❌ Error merging from schools table: $e');
  ...
  Future<void> _saveMergedData() async {
    try {
      ...
      debugPrint('✅ Merged settings saved to database');
    } catch (e) {
      debugPrint('❌ Error saving merged data: $e');
    }
  }
  ...
  Future<void> saveAllBusinessSettings() async {
    try {
      debugPrint('💾 Starting saveAllBusinessSettings (enhanced)...');
      await saveBusinessSettings();
      debugPrint('✅ Enhanced business settings saved successfully');
      ...
      try {
        if (Get.isRegistered<SchoolConfigService>()) {
          ...
          debugPrint('🏫 School configuration updated');
        }
      } catch (e) {
        debugPrint('⚠️ Could not update school configuration: $e');
      }
    } catch (e) {
      debugPrint('❌ Error in saveAllBusinessSettings (enhanced): $e');
      rethrow;
    }
  }
  ...
  Future<bool> _hasValidApiToken() async {
    try {
      ...
      debugPrint('⚠️ Error checking API token: $e');
      return false;
    }
  }
  ...
  Future<void> fetchBusinessSettingsFromServer() async {
    try {
      ...
      debugPrint('📥 Received business data from server: $businessData');
      ...
      debugPrint('✅ Business settings fetched and cached successfully');
      ...
      debugPrint('❌ Error fetching business settings from server: $e');
      ...
      if (errorString.contains('not authenticated') ||
        ...
        debugPrint('ℹ️ Authentication required - skipping error notification');
  ...
  Future<void> _saveBusinessSettingsToLocalCache(
    ...
    try {
      ...
      debugPrint('💾 Business settings cached to local database');
    } catch (e) {
      debugPrint('❌ Error caching business settings: $e');
  ...
  void toggleMultiTenant(bool enabled) {
    ...
    if (enabled) {
      debugPrint('🏫 Multi-tenant mode enabled');
      ...
      try {
        ...
        debugPrint('⚠️ Could not initialize school config: $e');
      }
    } else {
      debugPrint('🏫 Multi-tenant mode disabled');
    }
  }
  ...
  void toggleCloudSync(bool enabled) {
    ...
    debugPrint('☁️ Cloud sync ${enabled ? "enabled" : "disabled"}');
  }
  ...
  Future<void> _setDefaultSettings() async {
    debugPrint('⚠️ Setting default values due to loading failure');
...
// FIXED: Proper async save method
  Future<void> _saveSetting(String key, dynamic value) async {
    try {
      ...
      debugPrint('✅ Successfully saved $key = $value to SharedPreferences');
    } catch (e) {
      debugPrint('❌ Error saving setting $key to SharedPreferences: $e');
  ...
  void toggleAutoAssignVehicles(bool value) async {
    try {
      debugPrint('🔧 Toggling auto assign vehicles to: $value');
      await updateSetting('auto_assign_vehicles', value, autoAssignVehicles);
    } catch (e) {
      debugPrint('❌ Failed to toggle auto assign vehicles: $e');
...
// Repeat for all other toggle methods...
  ...
  Future<void> debugSharedPreferences() async {
    try {
      ...
      debugPrint('Total keys: ${keys.length}');
      ...
      debugPrint('🔍 === END SHARED PREFERENCES DEBUG ===');
    } catch (e) {
  ...
  void _showSettingUpdatedSnackbar(String key) {
    ...
    if (_isInitialized.value && Get.context != null) {
      try {
        ...
        debugPrint('⚠️ Could not show snackbar for $key: $e');
      }
    } else {
      debugPrint(
          'ℹ️ Setting $key updated (snackbar skipped during initialization)');
    }
  }
  ...
  Future<void> updateSetting(
    ...
    try {
      debugPrint('💾 Updating setting: $key = $value');
      ...
      _saveSettingToDatabase(key, value).catchError((e) {
        debugPrint('⚠️ Database backup failed for $key: $e');
      });
      ...
      debugPrint('✅ Setting $key saved successfully');
    } catch (e) {
      ...
      if (observableValue is Rx) {
        ...
        debugPrint('⚠️ Consider reverting $key due to save failure');
      }
  ...
  void toggleBillingValidation(bool value) async {
    try {
      debugPrint('🔧 Toggling billing validation to: $value');
      ...
      _saveSettingToDatabase('enforce_billing_validation', value)
          .catchError((e) {
        debugPrint('⚠️ Database backup failed: $e');
      });
      ...
      debugPrint('✅ Billing validation saved: $value');
    } catch (e) {
      debugPrint('❌ Failed to toggle billing validation: $e');
  ...
  void toggleInstructorAvailabilityCheck(bool value) async {
    try {
      debugPrint('🔧 Toggling instructor availability to: $value');
      ...
      debugPrint('✅ Instructor availability saved: $value');
    } catch (e) {
      debugPrint('❌ Failed to toggle instructor availability: $e');
      checkInstructorAvailability.value = !value;
    }
  }
  ...
  void toggleWorkingHours(bool value) async {
    try {
      debugPrint('🔧 Toggling working hours to: $value');
      ...
      debugPrint('✅ Working hours saved: $value');
    } catch (e) {
      debugPrint('❌ Failed to toggle working hours: $e');
      enforceWorkingHours.value = !value;
    }
  }
  ...
  Future<void> _initializeSettingsWithDefaults() async {
    try {
      debugPrint('🚀 Starting settings initialization with defaults...');
      ...
      if (!verified) {
        debugPrint('⚠️ Settings verification failed, forcing save...');
        await _saveAllSettingsToPreferences();
      }
      ...
      if (schoolId.value.isNotEmpty) {
        ...
        if (hasApiToken) {
          ...
          try {
            ...
            debugPrint('✅ Business settings fetched and updated from server');
          } catch (e) {
            debugPrint('⚠️ Failed to fetch business settings from server: $e');
      ...
      debugPrint('✅ Settings initialized with defaults successfully');
    } catch (e) {
      debugPrint('❌ Error initializing settings: $e');
  ...
  Future<void> _saveSettingToDatabase(String key, dynamic value) async {
    try {
      ...
      debugPrint('✅ Setting $key backed up to database');
    } catch (e) {
      debugPrint('⚠️ Failed to backup setting $key to database: $e');
  ...
  void enableSnackbars() {
    _isInitialized.value = true;
    debugPrint('📢 Snackbars enabled for settings updates');
  }

  void disableSnackbars() {
    _isInitialized.value = false;
    debugPrint('🔇 Snackbars disabled for settings updates');
  }

  Future<bool> verifySettingsPersistence() async {
    try {
      debugPrint('🔍 Verifying settings persistence...');
      final prefs = await SharedPreferences.getInstance();
      ...
      if (isMatch) {
        debugPrint('✅ Settings persistence verified');
        return true;
      } else {
        debugPrint('❌ Settings persistence FAILED');
        debugPrint(
      ...
      debugPrint('❌ Error verifying settings persistence: $e');
      return false;
    }
  }
  ...
  Future<void> testSettingsPersistence() async {
    ...
    debugPrint('🧹 Cleared test settings');
    enforceBillingValidation.value = true;
    checkInstructorAvailability.value = true;
    enforceWorkingHours.value = true;
    ...
    if (billing && instructor && hours) {
      debugPrint('✅ Settings persistence test PASSED');
    } else {
      debugPrint('❌ Settings persistence test FAILED');
      await debugSharedPreferences();
    }

    debugPrint('🧪 === END PERSISTENCE TEST ===');
  }

  // OPTIONAL: Method to force fix all settings to proper defaults
  Future<void> forceFixAllSettings() async {
    debugPrint('🔧 Force fixing all settings to proper defaults...');
    ...
    try {
      ...
      debugPrint('✅ All settings forced to proper defaults');

      // Verify the fix worked
      await debugSharedPreferences();
    } catch (e) {
      debugPrint('❌ Error force fixing settings: $e');
    }
  }
  ...
  Future<void> _saveAllSettingsToPreferences() async {
    try {
      ...
      debugPrint(
          '✅ All settings saved to SharedPreferences with proper defaults');
    } catch (e) {
  ...
  Future<void> loadPrinterSettings() async {
    try {
      final db = await _dbHelper.database;
      debugPrint('📖 Loading printer settings...');
      ...
      debugPrint('✅ Printer settings loaded successfully');
    } catch (e) {
...
// ============ SAVE PRINTER SETTINGS ============
  ...
  Future<void> savePrinterSettings() async {
    try {
      ...
      debugPrint('✅ Printer settings saved successfully');
    } catch (e) {
...
Future<void> quickFixSettings() async {
  ...
  debugPrint('🚀 Running quick settings fix...');
