...
class ScheduleController extends GetxController {
  ...
  void onReady() {
    ...
    try {
      ...
      debugPrint('BillingController not found: $e');
    }
    ...
    try {
      ...
      for (final scheduleData in data) {
        try {
          ...
          debugPrint('Error parsing schedule ${scheduleData['id']}: $e');
  ...
  dynamic _getCourseById(int id) {
    try {
      ...
      debugPrint('CourseController not found: $e');
      return null;
    }
  }
...
// Helper method to check availability with optional exclusion
  ...
  Future<bool> checkBillingLessons(
    ...
    try {
      ...
      debugPrint('Error checking billing lessons: $e');
      return false;
    }
  }
...
// Method to get maximum selectable date (1 year from now)
  ...
  Future<void> _updateBillingRecordStatus(
    ...
    try {
      ...
      debugPrint('Error updating billing record: $e');
  ...
  Future<void> _createBillingRecordIfNeeded(Schedule schedule) async {
    try {
      ...
      debugPrint('Error creating billing record: $e');
  ...
  Future<void> runStatusMigration() async {
    try {
      ...
      debugPrint('Pre-migration consistency check completed');
  ...
  Future<void> runConsistencyCheck() async {
    try {
      ...
      debugPrint('=== CONSISTENCY CHECK RESULTS ===');
      debugPrint(report);
      ...
      debugPrint('Error running consistency check: $e');
      Get.snackbar(
          snackPosition: SnackPosition.BOTTOM,
          'Check Error',
...
// 2. getUsedLessons method (similar logic but used elsewhere)
  ...
  Future<void> refreshBillingData() async {
    try {
      ...
      debugPrint('✓ Billing data refreshed in ScheduleController');
    } catch (e) {
  ...
  int getRemainingLessons(int studentId, int courseId) {
    ...
    debugPrint('DEBUG: Student $studentId, Course $courseId');
    debugPrint('Found ${invoices.length} invoices for student');
    for (var i = 0; i < invoices.length; i++) {
      debugPrint('  Invoice ${i + 1}: ${invoices[i].lessons} lessons');
    }
    debugPrint('Total lessons across all invoices: $totalLessons');
    debugPrint('Used lessons: $usedLessons');
    debugPrint('Remaining lessons: $remaining');
...
// DEBUGGING METHOD: Add this to help troubleshoot
  void debugBillingAndSchedules(int studentId, int courseId) {
    ...
    for (var schedule in studentSchedules) {
      debugPrint('Schedule ID: ${schedule.id}, Status: ${schedule.status}, '
  ...
  Future<void> addOrUpdateSchedule(Schedule schedule,
    ...
    try {
      ...
      if (schedule.id == null) {
        ...
        debugPrint('📝 Tracked schedule creation for sync');
        ...
        debugPrint('📝 Tracked schedule update for sync');
...
// 2. In deleteSchedule method - track schedule deletion
  Future<void> deleteSchedule(int scheduleId) async {
    try {
      ...
      debugPrint('📝 Tracked schedule deletion for sync');
...
// 3. In cancelSchedule method - track schedule status change
  Future<void> cancelSchedule(int scheduleId) async {
    try {
      ...
      debugPrint('📝 Tracked schedule cancellation for sync');
      ...
      try {
        ...
        debugPrint('Error handling billing record: $e');
      }
  ...
  Future<void> rescheduleSchedule(int scheduleId, Schedule newSchedule) async {
    try {
      ...
      if (index != -1) {
        ...
        await SyncService.trackChange('schedules', updateData, 'update');
        debugPrint('📝 Tracked original schedule reschedule status for sync');
...
// 5. In toggleAttendance method - track attendance and status changes
  Future<void> toggleAttendance(int scheduleId, bool attended) async {
    try {
      ...
      await SyncService.trackChange('schedules', updateData, 'update');
      debugPrint('📝 Tracked schedule attendance toggle for sync');
  ...
  Future<void> createMultipleSchedules(List<Schedule> schedulesToCreate) async {
    try {
      ...
      for (final schedule in schedulesToCreate) {
        ...
        await SyncService.trackChange(
            'schedules', createdSchedule.toJson(), 'create');
        debugPrint('📝 Tracked bulk schedule creation for sync');
      ...
      debugPrint('Error in bulk schedule creation: $e');
      Get.snackbar(
          snackPosition: SnackPosition.BOTTOM,
          'Bulk Creation Error',
