...
class SubscriptionGuard extends GetMiddleware {
  ...
  GetPage? onPageCalled(GetPage? page) {
    try {
      ...
      final hasLoadedData =
          subscriptionController.subscriptionStatus.value.isNotEmpty &&
      ...
      if (hasLoadedData) {
        ...
        if (status == 'suspended' || status == 'expired') {
          debugPrint('🚫 BLOCKED: Status is $status');
          return GetPage(
            name: route ?? '/checking',
        ...
        debugPrint('✅ Subscription OK ($status) - allowing access to: $route');
      ...
      debugPrint('⚠️ No subscription data - checking...');
      return GetPage(
        name: route ?? '/checking',
        page: () => _SubscriptionCheckingScreen(
          targetRoute: route ?? '/main',
      ...
      debugPrint('❌ Subscription Guard Error: $e');
  ...
  static void markCheckCompleted() {
    ...
    debugPrint(
        '✅ Subscription check completed - cache valid for ${CACHE_DURATION.inMinutes} minutes');
  }
      ...
      if (!hasExistingData) {
        ...
        if (subscriptionController.subscriptionStatus.value.isEmpty ||
          ...
          try {
            ...
            debugPrint('⚠️ Server load failed/timeout: $e');
            // Continue with whatever we have
          }
        }
      } else {
        debugPrint('✅ Using existing subscription data');
      }

      final status = subscriptionController.subscriptionStatus.value;
      final trialDays = subscriptionController.remainingTrialDays.value;

      debugPrint('📊 Status: $status, Trial: $trialDays');

      SubscriptionGuard.markCheckCompleted();

      // ✅ Only block for definite invalid states
      if (status == 'suspended' || status == 'expired') {
        debugPrint('🚫 BLOCKED: $status');
        _showBlockedScreen(status);
        return;
      }

      if (status == 'trial' && trialDays <= 0) {
        debugPrint('🚫 BLOCKED: Trial expired');
        _showBlockedScreen('trial_expired');
        return;
      }

      // ✅ Allow access
      debugPrint('✅ ALLOWED: Proceeding to ${widget.targetRoute}');
